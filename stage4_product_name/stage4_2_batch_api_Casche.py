...
Stage 4-2 Batch API 실행 스크립트 (GUI) - 캐싱 최적화 버전
- 기능: Batch JSONL 생성 -> 업로드 -> 실행 -> 결과 병합 -> 상세 리포트
- [Update] 툴팁(Tooltip) 추가, 동작 순서(Step) UI 명확화
...
# ========================================================
# 메인 런처 연동용 JobManager & 파일명 유틸 (Stage4-2: Text)
# ========================================================
def get_root_filename(filename: str) -> str:
    ...
    예: 상품_T4_I0(업완).xlsx -> 상품.xlsx
    예: 상품_T4_I0_T4(완)_I1.xlsx -> 상품.xlsx (여러 버전 패턴 제거)
    예: 상품_T4_I5(업완).xlsx -> 상품.xlsx
    """
...
def get_next_version_path(current_path: str, task_type: str = "text") -> str:
    ...
    - task_type='text'  → T 버전 +1 (Stage1: T1, Stage2: T2, Stage3: T3, Stage4: T4, ...)
    - task_type='image' → I 버전 +1
    예: 상품_T3_I5(업완).xlsx -> 상품_T4_I5(업완).xlsx (text) 또는 상품_T3_I6(업완).xlsx (image)
    """
    dir_name = os.path.dirname(current_path)
    base_name = os.path.basename(current_path)
    name_only, ext = os.path.splitext(base_name)
    
    # 괄호 포함 패턴 매칭 (예: _I5(업완))
    pattern = r"_T(\d+)_I(\d+)(\([^)]+\))?$"
...
def get_t4_complete_path(current_path: str) -> str:
    """
    Stage 4-2 완료 후 파일명을 T4(완)_I* 형식으로 생성합니다.
    예: 상품_T3_I1.xlsx -> 상품_T4(완)_I1.xlsx
    예: 상품_T4_I5(업완).xlsx -> 상품_T4(완)_I5(업완).xlsx
    
    주의: I 버전과 (업완) 등 모든 후속 정보를 그대로 유지합니다.
    """
    dir_name = os.path.dirname(current_path)
    base_name = os.path.basename(current_path)
    name_only, ext = os.path.splitext(base_name)
    
    # _T*_I 이후의 모든 부분을 추출 (I 버전과 후속 정보 포함)
    # 예: "_T4_I5(업완)" -> "_I5(업완)" 추출
    pattern = r"_T\d+_I(\d+)(.*?)(?:_[Tt]\d+_[Ii]\d+|$)"
    match = re.search(pattern, name_only, re.IGNORECASE)
    
    if match:
        current_i = int(match.group(1))
        i_suffix = match.group(2)  # I 버전 이후의 모든 부분 (예: (업완))
        # 원본명은 패턴 이전까지
        original_name = name_only[: match.start()].rstrip("_")
        # T4(완)_I* 형식으로 저장 (I 버전과 후속 정보 그대로 유지)
        new_filename = f"{original_name}_T4(완)_I{current_i}{i_suffix}{ext}"
    else:
        # 패턴이 없으면 원본명에서 버전 정보 제거 후 사용
        original_name = name_only
        # 기존 버전 패턴 제거
        while True:
            new_name = re.sub(r"_[Tt]\d+_[Ii]\d+.*?(?:_[Tt]\d+_[Ii]\d+|$)", "", original_name, flags=re.IGNORECASE)
            if new_name == original_name:
                break
            original_name = new_name
        original_name = original_name.rstrip("_")
        new_filename = f"{original_name}_T4(완)_I0{ext}"
...
# [필수 의존성] stage4_2_core_Casche.py / stage4_2_run_history.py
# 캐싱 최적화 버전 사용 (stage4_2_core_Casche.py)
try:
    from stage4_2_core_Casche import (
        safe_str,
        build_stage4_2_batch_payload,  # Row -> Batch Payload 변환 (캐싱 최적화)
    ...
    try:
        ...
        def build_stage4_2_batch_payload(*args, **kwargs): return None
        def append_run_history(*args, **kwargs): pass

# === 기본 설정 ===
API_KEY_FILE = ".openai_api_key_stage4_batch"
BATCH_JOBS_FILE = os.path.join(os.path.dirname(__file__), "stage4_2_batch_jobs.json")
...
def save_batch_jobs(jobs):
    try:
        with open(BATCH_JOBS_FILE, "w", encoding="utf-8") as f: json.dump(jobs, f, indent=2, ensure_ascii=False)
...
# GUI Class
# ========================================================
class Stage4BatchGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stage 4-2: Batch API Manager (Tooltip & Workflow) 🚀 캐싱 최적화 버전")
    ...
    def _init_ui(self):
        ...
        # 1. 상단 API Key
        f_top = ttk.LabelFrame(main_container, text="🔑 기본 설정", padding=10)
        ...
        ToolTip(btn_save, "입력한 API 키를 로컬(.openai_api_key_stage4_batch)에 저장합니다.")
        ...
        self.main_tabs.add(self.tab_create, text=" 1. 배치 생성 & 업로드 (Create) ")
        self.main_tabs.add(self.tab_manage, text=" 2. 배치 관리 (Manage) ")
        self.main_tabs.add(self.tab_merge, text=" 3. 개별 수동 작업 (Manual) ")
        ...
        self.log_widget = ScrolledText(f_log, height=18, state='disabled', font=("Consolas", 9), bg="#F1F3F5")
    ...
    def _init_tab_create(self):
        ...
        # Step 1: 파일 선택
        f_step1 = ttk.LabelFrame(container, text="Step 1. 대상 파일 선택", padding=15)
        f_step1.pack(fill='x', pady=(0, 15))
        
        ttk.Entry(f_step1, textvariable=self.src_file_var, font=("맑은 고딕", 10)).pack(side='left', fill='x', expand=True)
        btn_file = ttk.Button(f_step1, text="📂 파일 찾기", command=self._select_src_file)
        btn_file.pack(side='right', padx=5)
        ToolTip(btn_file, "Stage 3(또는 4-1)까지 완료되어 'ST3_결과상품명' 컬럼이 있는\n엑셀 파일을 선택하세요.")
        
        # Step 2: 옵션 설정
        f_step2 = ttk.LabelFrame(container, text="Step 2. 배치 옵션 설정", padding=15)
        f_step2.pack(fill='x', pady=5)
        
        f_row1 = ttk.Frame(f_step2)
        f_row1.pack(fill='x', pady=5)
        ttk.Label(f_row1, text="모델 (Model):", width=12).pack(side='left')
        models = list(MODEL_PRICING_USD_PER_MTOK.keys())
        cb_model = ttk.Combobox(f_row1, textvariable=self.model_var, values=models, state="readonly", width=20)
        cb_model.pack(side='left', padx=5)
        ToolTip(cb_model, "사용할 AI 모델을 선택합니다.\n(gpt-5-mini 추천: 가성비 우수)")
        
        ttk.Label(f_row1, text="추론 강도 (Effort):").pack(side='left', padx=(20, 5))
        cb_effort = ttk.Combobox(f_row1, textvariable=self.effort_var, values=["low", "medium", "high"], state="readonly", width=12)
        cb_effort.pack(side='left', padx=5)
        ToolTip(cb_effort, "AI가 얼마나 깊게 생각할지 설정합니다.\n- Low: 빠름\n- Medium: 균형\n- High: 정밀함 (비용 증가)")
        
        f_row2 = ttk.Frame(f_step2)
        f_row2.pack(fill='x', pady=5)
        chk_skip = ttk.Checkbutton(f_row2, text=" 이미 결과(ST4)가 있는 행은 건너뛰기 (Skip)", variable=self.skip_exist_var)
        chk_skip.pack(side='left')
        ToolTip(chk_skip, "체크 시: 이미 'ST4_최종결과' 값이 있는 행은 API를 호출하지 않습니다.\n(중복 과금 방지)")
        
        # Step 3: 실행
        f_step3 = ttk.LabelFrame(container, text="Step 3. 실행", padding=15)
        ...
        ToolTip(btn_run, "1. 엑셀 데이터를 읽어 JSONL 파일 생성\n2. OpenAI 서버에 파일 업로드\n3. 배치 작업 시작 요청 (비용 50% 절감)\n\n※ 결과는 최대 24시간 후 2번 탭에서 확인 가능합니다.")
        
        ttk.Label(container, text="※ 배치 API는 비동기 방식이므로 즉시 결과가 나오지 않습니다.\n(보통 1~3시간 소요)\n 최대 24시간 이내", foreground="#666").pack()
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            cand_col = 'ST3_결과상품명'
            for c in df.columns:
                if 'filtered' in c or '정제결과' in c: cand_col = c
            
            if cand_col not in df.columns:
                raise ValueError(f"후보군 컬럼({cand_col}) 없음")
            
            # 캐싱 모드 확인 및 로그
            if CACHE_MODE_CORE:
                self.append_log(f"[INFO] 🚀 프롬프트 캐싱 최적화 모드 활성화 (stage4_2_core_Casche.py)")
                # System prompt 토큰 수 확인
                try:
                    import tiktoken
                    from stage4_2_core_Casche import STAGE4_2_SYSTEM_PROMPT
                    enc = tiktoken.encoding_for_model('gpt-4o')
                    token_count = len(enc.encode(STAGE4_2_SYSTEM_PROMPT))
                    char_count = len(STAGE4_2_SYSTEM_PROMPT)
                    status = "✅ 충분" if token_count > 1024 else "⚠️ 부족"
                    diff = token_count - 1024
                    self.append_log(f"[INFO] System prompt: {char_count:,}자, {token_count} 토큰 ({status}, 기준: 1024 토큰, {diff:+d} 토큰)")
                    if token_count <= 1024:
                        self.append_log(f"[WARN] ⚠️ System prompt가 1024 토큰 미만입니다. 프롬프트 캐싱이 제대로 작동하지 않을 수 있습니다.")
                        self.append_log(f"[WARN] ⚠️ User prompt의 일부가 첫 1024 토큰에 포함되어 캐시가 깨질 수 있습니다.")
                except Exception as e:
                    self.append_log(f"[WARN] System prompt 토큰 수 확인 실패: {e}")
            else:
                self.append_log(f"[INFO] ⚠️ 일반 모드 (stage4_2_core.py) - 캐싱 최적화 미적용")
            
            # 먼저 전체 대상 요청 수를 계산 (버킷 수 결정용)
            target_rows = 0
            for idx, row in df.iterrows():
                if self.skip_exist_var.get() and "ST4_최종결과" in df.columns:
                    val = str(row.get("ST4_최종결과", "")).strip()
                    if val and val != "nan":
                        continue
            ...
            if CACHE_MODE_CORE and target_rows > 0:
                ...
                self.append_log(f"[INFO] 프롬프트 캐싱: 키 고정 전략 사용 (모든 요청이 'stage4_2_v1' 키 공유)")
            ...
            for idx, row in df.iterrows():
                if self.skip_exist_var.get() and "ST4_최종결과" in df.columns:
                    val = str(row.get("ST4_최종결과", "")).strip()
                    if val and val != "nan": continue
                
                payload = build_stage4_2_batch_payload(
                    idx, row, model, effort, cand_col,
                ...
                if CACHE_MODE_CORE and "body" in payload:
                    custom_id = payload.get("custom_id", f"row_{idx}")
                    ...
                    payload["body"]["prompt_cache_key"] = "stage4_2_v1"
                    ...
                    if model in ["gpt-5.1", "gpt-5.1-codex", "gpt-5.1-codex-mini", "gpt-5.1-chat-latest", "gpt-5", "gpt-5-codex", "gpt-4.1"]:
                        payload["body"]["prompt_cache_retention"] = "extended"  # 24시간 retention
                    elif model not in ["gpt-5-mini", "gpt-5-nano"]:
                        # 기타 모델은 in-memory 사용 (5~10분 inactivity, 최대 1시간)
                        payload["body"]["prompt_cache_retention"] = "in_memory"
            ...
            jsonl_path = f"{base}_stage4_batch_input.jsonl"
            with open(jsonl_path, "w", encoding="utf-8") as f: f.write("\n".join(jsonl_lines))
            ...
            if jsonl_size_mb > MAX_FILE_SIZE_MB:
                ...
                with open(jsonl_path, "rb") as f: batch_input_file = client.files.create(file=f, purpose="batch")
                ...
                # 메인 런처 현황판에 Stage4-2(Text) 작업 시작 상태 기록: T4-2 (진행중) (img 상태는 변경하지 않음)
                try:
                    root_name = get_root_filename(src)
                    JobManager.update_status(root_name, text_msg="T4-2 (진행중)")
                    self.append_log(f"[INFO] 런처 상태 업데이트: {root_name} -> T4-2 (진행중)")
                except Exception:
                    # 런처나 job_history.json 이 없을 수 있으므로 조용히 무시
                    pass
                
                messagebox.showinfo("성공", f"배치 작업이 시작되었습니다.\nID: {batch_id}\n\n[2. 배치 관리] 탭에서 상태를 확인하세요.")
                ...
                try:
                    ...
                    if retry_count < max_retries:
                        ...
                        if is_token_limit_error:
                            ...
                            self.append_log(f"[INFO] 나중에 '3. 개별 수동 작업 (Manual)' 탭에서 이 파일을 사용하여 재시도할 수 있습니다.")
                        import traceback
    ...
    def _init_tab_manage(self):
        ...
        btn_refresh = ttk.Button(f_ctrl, text="🔄 선택 갱신", command=lambda: self._refresh_selected(self.tree_active))
        btn_refresh.pack(side='left', padx=2)
        ToolTip(btn_refresh, "선택한 작업의 현재 상태(진행중/완료/실패)를\nOpenAI 서버에서 확인하여 갱신합니다.")
        
        btn_merge = ttk.Button(f_ctrl, text="📥 선택 일괄 병합", command=self._merge_selected, style="Primary.TButton")
        btn_merge.pack(side='left', padx=10)
        ToolTip(btn_merge, "[완료(completed)] 상태인 작업을 선택하여\n결과를 다운로드하고 원본 엑셀에 병합합니다.")
        
        btn_report = ttk.Button(f_ctrl, text="📊 선택 일괄 통합 리포트", command=self._report_selected_unified, style="Success.TButton")
        btn_report.pack(side='left', padx=2)
        ToolTip(btn_report, "[병합됨(merged)] 상태인 작업들을 선택하여\n'삭제된 내역' 등을 분석한 하나의 통합 엑셀 리포트를 생성합니다.")
        
        btn_trash = ttk.Button(f_ctrl, text="🗑 휴지통 이동", command=self._archive_selected, style="Danger.TButton")
        btn_trash.pack(side='right', padx=2)
        ToolTip(btn_trash, "선택한 작업을 목록에서 숨기고 휴지통으로 보냅니다.")
        ...
        btn_restore = ttk.Button(f_arch_ctrl, text="♻️ 선택 복구", command=self._restore_selected, style="Primary.TButton")
        btn_restore.pack(side='left')
        ToolTip(btn_restore, "선택한 작업을 다시 Active 목록으로 되돌립니다.")
        
        btn_del = ttk.Button(f_arch_ctrl, text="🔥 선택 영구 삭제", command=self._hard_delete_selected, style="Danger.TButton")
        btn_del.pack(side='right')
        ToolTip(btn_del, "선택한 작업을 로컬 DB에서 영구히 삭제합니다.\n(OpenAI 서버의 기록은 삭제되지 않음)")
    ...
    def _load_jobs_all(self):
        ...
        for group_id, group_jobs in sorted(grouped_jobs.items()):
            ...
            excel_name = os.path.basename(first_job.get("src_excel", "")) if first_job.get("src_excel") else "-"
    ...
    def _refresh_selected(self, tree):
        ...
        completed = []
        for bid in ids:
            j = next((x for x in jobs if x["batch_id"] == bid), None)
            if j and j.get("status") in ["completed", "merged"]:
                completed.append(bid)
        
        if completed:
            msg = (f"선택한 항목 중 {len(completed)}건은 이미 완료되었습니다.\n"
                   "완료된 항목은 제외하고 갱신하시겠습니까?\n\n"
                   "- 예(Yes): 미완료 항목만 갱신\n- 아니오(No): 전체 강제 갱신")
            if messagebox.askyesno("중복 갱신 방지", msg):
            ...
            try:
                ...
                for bid in batch_ids_sorted:
                    ...
                    try:
                        ...
                        out_jsonl = os.path.join(base_dir, f"{base_name}_stage4_batch_output_{bid}.jsonl")
                    ...
                    for bid in batch_ids_sorted:
                        ...
                        if local_job and local_job.get("status") in ["completed", "expired"]:
                            out_jsonl = local_job.get("output_jsonl") or os.path.join(
                                ...
                                f"{os.path.splitext(os.path.basename(src_path))[0]}_stage4_batch_output_{bid}.jsonl"
                            )
                ...
                out_excel = f"{base}_stage4_batch_done.xlsx"
                
                df = pd.read_excel(src_path)
                if "ST4_최종결과" not in df.columns:
                    df["ST4_최종결과"] = ""
                df["ST4_최종결과"] = df["ST4_최종결과"].astype(str)
                
                cnt = 0
                for cid, val in all_results_map.items():
                    try:
                        # custom_id 형식: "row_0", "row-0" 등
                        if cid.startswith("row-"):
                            ...
                            # 기타 형식 시도
                            idx = int(cid.split("_")[1]) if "_" in cid else int(cid.split("-")[1])
                        
                        if 0 <= idx < len(df):
                            df.at[idx, "ST4_최종결과"] = val
                            cnt += 1
                    except Exception as e:
                ...
                # Stage4 최종 파일명: *_T4(완)_I* 형식으로 저장
                try:
                    final_out_path = get_t4_complete_path(src_path)
                    df_done = pd.read_excel(out_excel)
                    if safe_save_excel(df_done, final_out_path):
                        out_path_for_history = final_out_path
                        self.append_log(f"  [그룹] 최종 파일 저장: {os.path.basename(final_out_path)}")
                        # T4(완) 버전 파일 저장 성공 시, 코어가 생성한 중간 파일(_stage4_batch_done) 삭제
                    ...
                    self.append_log(f"[WARN] T4(완) 버전 파일 저장 중 오류: {e}")
                    out_path_for_history = out_excel
                ...
                if c_at_str:
                    try:
                        ...
                        # 시간대 정보가 있는 경우 get_seoul_now()와 같은 시간대로 변환
                        if c_at.tzinfo is not None:
                            try:
                                from pytz import timezone
                                seoul_tz = timezone("Asia/Seoul")
                                c_at = c_at.astimezone(seoul_tz)
                            except:
                                # pytz가 없으면 naive로 변환
                                c_at = c_at.replace(tzinfo=None)
                        # 시간대 정보가 없는 경우 (naive datetime) get_seoul_now()도 naive로 변환하여 계산
                    except:
                        c_at = get_seoul_now()
                else:
                    c_at = get_seoul_now()
                ...
                now = get_seoul_now()
                # 시간대 정보 일치 확인 후 계산
                if c_at.tzinfo is None or now.tzinfo is None:
                    # 둘 중 하나라도 naive면 둘 다 naive로 변환
                    if c_at.tzinfo is not None:
                        c_at = c_at.replace(tzinfo=None)
                    if now.tzinfo is not None:
                        now = now.replace(tzinfo=None)
                
                elapsed_seconds = (now - c_at).total_seconds()
                
                append_run_history(
                    stage="Stage 4-2 Batch",
                    model_name=model_name,
                    ...
                    input_cost_usd=(total_group_in / 1_000_000) * MODEL_PRICING_USD_PER_MTOK.get(model_name, {"input": 0, "output": 0})["input"] * 0.5,
                    output_cost_usd=(total_group_out / 1_000_000) * MODEL_PRICING_USD_PER_MTOK.get(model_name, {"input": 0, "output": 0})["output"] * 0.5,
                ...
                try:
                    ...
                    self.append_log(f"[INFO] 런처 현황판 업데이트: {root_name} -> T4-2(최종완료)")
                except Exception as e:
                ...
                self.append_log(f"  [그룹] 병합 완료 ({cnt}건): {os.path.basename(out_path_for_history)}")
                ...
                success_cnt += 1
                total_in += total_group_in
                total_out += total_group_out
        ...
        for bid in ungrouped_batches:
            ...
            try:
                ...
                if local_job and local_job.get("src_excel"):
                    ...
                    base, _ = os.path.splitext(src_path)
                    out_jsonl = f"{base}_stage4_batch_output.jsonl"
                    out_excel = f"{base}_stage4_batch_done.xlsx"
                else:
                ...
                total_in += batch_in_tok
                total_out += batch_out_tok
                total_cost += cost_total

                if src_path and os.path.exists(src_path):
                    df = pd.read_excel(src_path)
                    if "ST4_최종결과" not in df.columns: df["ST4_최종결과"] = ""
                    df["ST4_최종결과"] = df["ST4_최종결과"].astype(str)
                    cnt = 0
                    ...
                    try:
                        ...
                        if safe_save_excel(df_done, final_out_path):
                            out_path_for_history = final_out_path
                            self.append_log(f"[INFO] 최종 파일 저장: {os.path.basename(final_out_path)}")
                            # T4(완) 버전 파일 저장 성공 시, 코어가 생성한 중간 파일(_stage4_batch_done) 삭제
                            if out_excel != final_out_path and os.path.exists(out_excel):
                                try:
                                    os.remove(out_excel)
                    ...
                    # 메인 런처 현황판에 Stage4-2(Text) 완료 상태 기록: T4-2(최종완료) (img 상태는 변경하지 않음)
                    try:
                        root_name = get_root_filename(src_path)
                        JobManager.update_status(root_name, text_msg="T4-2(최종완료)")
                        self.append_log(f"[INFO] 런처 현황판 업데이트: {root_name} -> T4-2(최종완료)")
    ...
    def _report_selected_unified(self):
        ...
        if not targets:
            messagebox.showinfo("알림", "리포트를 생성하려면 상태가 'merged'여야 합니다.")
            return
        if messagebox.askyesno("리포트", f"선택한 {len(targets)}건의 리포트를 통합 생성합니까?"):
    ...
    def _run_report_unified(self, ids):
        ...
        for bid in ids:
            ...
            if not src or not out or not os.path.exists(src) or not os.path.exists(out): continue
            try:
                ...
                for c in df_in.columns:
                    if 'filtered' in c or '정제결과' in c: cand_col = c
                res_col = 'ST4_최종결과'
                if res_col not in df_out.columns: continue
                for idx, row_in in df_in.iterrows():
                    p_code = safe_str(row_in.get('상품코드', ''))
                    st1 = safe_str(row_in.get('ST1_결과상품명', ''))
                    raw_st3 = safe_str(row_in.get(cand_col, ''))
                    cands = [x.strip() for x in raw_st3.split('\n') if x.strip()]
                    st4 = ""
                    if idx < len(df_out): st4 = safe_str(df_out.iloc[idx].get(res_col, ''))
                    results = [x.strip() for x in st4.split('\n') if x.strip()]
                    removed = [c for c in cands if c not in results]
                    removed_str = "\n".join(removed)
                    status = "✅ 최적화됨" if removed else ("변동 없음" if st4 else "❌ 공란")
                    all_diffs.append({
                        "Batch_ID": bid, "행번호": idx+2, "상품코드": p_code,
                        "ST1_기준명": st1, "입력수": len(cands), "출력수": len(results),
                        "삭제된_문구": removed_str, "ST4_최종결과": st4, "상태": status
                    })
            except: pass
        if not all_diffs:
        ...
        try:
            ...
            path = os.path.join(os.path.dirname(__file__), f"Integrated_Diff_{ts}.xlsx")
            pd.DataFrame(all_diffs).to_excel(path, index=False)
            self.append_log(f"📊 리포트 생성: {os.path.basename(path)}")
    ...
    def _init_tab_merge(self):
        ...
        btn_merge = ttk.Button(f_btn, text="1. 결과 병합", command=self._start_merge, style="Primary.TButton")
        btn_merge.pack(fill='x', pady=5)
        btn_report = ttk.Button(f_btn, text="2. 단일 리포트", command=self._start_diff_report)
        btn_report.pack(fill='x', pady=5)
        ToolTip(btn_merge, "단일 배치 작업의 결과를 다운로드하여 원본 엑셀에 병합합니다.")
    ...
    def _handle_failed_chunks(self, failed_info_path, failed_chunk_files):
        ...
        msg += f"• '3. 개별 수동 작업 (Manual)' 탭에서 재시도할 수 있습니다."
