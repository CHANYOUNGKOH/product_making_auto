"""
stage4_2_batch_api.py

Stage 4-2 Batch API 실행 스크립트 (GUI)
...
class Stage4BatchGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stage 4-2: Batch API Manager (Tooltip & Workflow)")
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                payload = build_stage4_2_batch_payload(idx, row, model, effort, cand_col)
                if payload: jsonl_lines.append(json.dumps(payload, ensure_ascii=False))
                else: skipped_cnt += 1
                    ...
                    try:
                        ...
                        with open(out_jsonl, "r", encoding="utf-8") as f:
                            for line in f:
                                ...
                                try:
                                    val = data["response"]["body"]["choices"][0]["message"]["content"]
                                    all_results_map[cid] = val.strip()
                                except:
                                    if cid:
                                        all_results_map[cid] = ""
