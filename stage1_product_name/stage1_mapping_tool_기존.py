    ...
    def on_run_thumbnail_mapping(self) -> None:
        ...
        def worker():
            try:
                ...
                def done():
                    self.current_df = df
                    self.current_out_path = out_excel
                    # 썸네일 작업 완료 후 *_stage1_img_mapping.xlsx 선택
                    self.excel_path_var.set(out_excel)
                    self._set_running_state(False)
                    self.set_status(f"썸네일 다운로드/맵핑 완료: {out_excel}")
                    self.refresh_preview()
                    messagebox.showinfo(
                        "완료",
                        "썸네일 다운로드 및 맵핑 작업이 완료되었습니다.\n"
                        f"엑셀: {out_excel}\n"
                        f"이미지 폴더: {img_dir}",
                    )
