...
# [stage1_mapping_tool.py] 의 JobManager 클래스

# [NEW] 파일명에서 꼬리표(_stage1 등)를 떼고 '뿌리 이름'만 남기는 함수
def get_root_filename(filename):
    name = os.path.basename(filename)
    base, ext = os.path.splitext(name)
    
    # 제거할 꼬리표들 (순서대로 제거 시도)
    suffixes = ["_stage1_mapping", "_stage1_img_mapping", "_stage1_IMG", "_stage2", "_stage3"]
...
class JobManager:
    # 초기값은 None으로 두고, 실제 사용할 때 찾습니다.
    DB_FILE = None

    @classmethod
    def find_db_path(cls):
        """job_history.json 파일을 찾아서 그 경로를 반환합니다."""
        # 1. 이미 찾았으면 그거 씀
        if cls.DB_FILE and os.path.exists(cls.DB_FILE):
            return cls.DB_FILE
        
        # 2. 현재 파일의 위치부터 시작해서 위로 올라가며 찾기
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # 현재 폴더 포함, 상위 3단계까지 검색
        search_dirs = [
            current_dir,                                      # 현재 폴더 (stage1_product_name)
            os.path.abspath(os.path.join(current_dir, "..")), # 상위 폴더 (상품가공프로그램 - 런처 위치)
            os.path.abspath(os.path.join(current_dir, "..", "..")) # 더 상위
        ]
        ...
        for d in search_dirs:
            ...
            if os.path.exists(target):
                cls.DB_FILE = target
                print(f"[JobManager] Found DB at: {target}")
                return target
        
        # 3. 그래도 없으면? -> 그냥 런처가 있을법한 '상위 폴더'에 새로 만듦
    ...
    def update_status(cls, filename, text_msg=None, img_msg=None):
        ...
        # [디버깅] 저장 위치를 콘솔에 출력
        print(f"[JobManager] Saving to: {db_path}")
        ...
        try:
            ...
            # [확인용] 팝업창은 너무 자주 뜨면 귀찮으니, print로만 확인하거나
            # 정말 안 될 때는 아래 주석을 풀어보세요.
            # messagebox.showinfo("저장 확인", f"현황판 업데이트 완료!\n경로: {db_path}")
            
        except Exception as e:
            print(f"[JobManager Error] 상태 저장 실패: {e}")
# 전체 배경 색
BG_MAIN = "#f4f4f7"
...
def process_excel_for_stage1(
    ...
    out_path = os.path.join(base_dir, f"{base_name}_stage1_mapping.xlsx")

    df.to_excel(out_path, index=False)
    log(f"[INFO] Stage1 맵핑 엑셀 저장 완료: {out_path}")
    return out_path, df
...
class Stage1MappingApp:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("Stage1 상품명 정제용 맵핑 파일 생성기 (대화방용)")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header,
            text="Stage1-1 상품명 정제 도구",
            bg="#e5edff",
            font=("맑은 고딕", 11, "bold"),
        ).pack(side=tk.LEFT, padx=10, pady=6)
        tk.Label(
            header,
            text="도매처 엑셀 → ST1 맵핑 파일 → 썸네일 매핑",
            bg="#e5edff",
            fg="#555555",
        ...
        desc = (
            ...
            "② Stage1 맵핑용 엑셀(*_stage1_mapping.xlsx)을 생성합니다.\n"
            "   (완료 시 자동으로 *_stage1_mapping.xlsx 파일이 선택됩니다.)\n"
            "③ '썸네일 다운로드/맵핑'을 누르면 이미지대 URL을 이용해\n"
            "   1000x1000 썸네일 JPG와 *_stage1_img_mapping.xlsx 을 생성합니다.\n"
            "   (완료 시 *_stage1_img_mapping.xlsx 파일이 자동 선택됩니다.)\n"
        )
        tk.Label(
            self.root,
            text=desc,
    ...
    def on_run_stage1(self) -> None:
        ...
        self.set_status("Stage1 맵핑 실행 중...")
        self.set_progress(0, 1)
        self.append_log(f"[INFO] Stage1 맵핑 실행 시작: {excel_path} ({source_key})")
        ...
        def worker():
            try:
                ...
                def done():
                    ...
                    # Stage1 맵핑 완료 후 자동으로 *_stage1_mapping.xlsx 선택
                    self.excel_path_var.set(out_path)
                    self._set_running_state(False)
                    self.set_status(f"Stage1 맵핑 완료: {out_path}")
                    ...
                    # 2. 뿌리 이름으로 상태 업데이트
                    JobManager.update_status(root_name, text_msg="Step 0 완료", img_msg="대기")  
                    messagebox.showinfo("완료", f"Stage1 맵핑 파일 생성 완료:\n{out_path}")

                self.root.after(0, done)
            except Exception as e:
                def err():
                    self._set_running_state(False)
                    self.set_status("Stage1 맵핑 실행 중 오류 발생")
                    messagebox.showerror("오류", f"Stage1 맵핑 실행 중 오류가 발생했습니다:\n{e}")
    ...
    def on_run_thumbnail_mapping(self) -> None:
        ...
        excel_path = self.excel_path_var.get().strip()
        if not excel_path:
            messagebox.showwarning("주의", "썸네일을 생성할 Stage1 맵핑 엑셀(또는 동일 구조의 엑셀)을 선택해주세요.")
            return
        if not os.path.exists(excel_path):
            messagebox.showerror("오류", f"파일을 찾을 수 없습니다:\n{excel_path}")
            return
        ...
        def worker():
            try:
                ...
                def done():
                    ...
                    # 2. 뿌리 이름으로 상태 업데이트
                    JobManager.update_status(root_name, img_msg="Step 0 완료")
                    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
