...
RUN_LOG_PATH = os.path.join(os.path.dirname(__file__), "stage1_run_log.xlsx")
...
def append_run_history(
    ...
    start_dt: datetime,
    finish_dt: datetime,
    # ----- 아래부터 신규 옵션 (기존 호출 코드 안 깨지게 전부 기본값 있음) -----
    ...
    """
    Stage1 실행 1회에 대한 요약 정보를 stage1_run_log.xlsx에 한 줄 추가.
    ...
    try:
        ...
        row = {
            "stage": stage,
            "api_type": api_type,  # 신규
            "batch_id": batch_id,  # 신규
            "start_time": start_dt.strftime("%Y-%m-%d %H:%M:%S"),
            "finish_time": finish_dt.strftime("%Y-%m-%d %H:%M:%S"),
            "elapsed_seconds": elapsed_seconds,
            "src_file": src_file,
            "out_file": out_file,  # 신규
            "total_rows": total_rows,
            "api_rows": api_rows,
            "success_rows": success_rows,  # 신규
            "fail_rows": fail_rows,        # 신규
