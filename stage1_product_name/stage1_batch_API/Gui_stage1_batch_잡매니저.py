...
# ========================================================
# [NEW] 메인 런처 연동용 JobManager & 유틸 (추가됨)
# ========================================================
def get_root_filename(filename):
    ...
    # 1. 버전 패턴 (_T숫자_I숫자) 제거
    # 정규식: _T 뒤에 숫자, _I 뒤에 숫자가 오는 패턴을 찾아서 제거
...
class Stage1BatchGUI(tk.Tk):
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            jsonl_path = os.path.join(os.path.dirname(src), f"{base_name}_stage1_batch.jsonl")
           
            with open(jsonl_path, "w", encoding="utf-8") as f:
                f.write("\n".join(jsonl_lines))
            
            self.append_log(f"JSONL 생성 완료: {len(jsonl_lines)}건")
            ...
            try:
                ...
                # T1 생성을 위한 작업이 시작되었으므로 'T1 (진행중)' 표기
    ...
    def _run_refresh_ids(self, ids, silent=False):
        ...
        # 요약 카운트
        completed_with_output = 0       # completed + output_file_id 있음
        completed_without_output = 0    # completed지만 output_file_id 없음
        not_completed = 0               # processing / validating / failed / cancelled 등
        ...
        try:
            ...
            for bid in ids:
                try:
                    ...
                    # request_counts 정리 (있을 때만)
                    rc = None
                    rc_obj = getattr(remote, "request_counts", None)
                    if rc_obj:
                        try:
                            rc = {
                                "total": rc_obj.total,
                                "completed": rc_obj.completed,
                                "failed": rc_obj.failed,
                            }
                        except Exception:
                            rc = None
                    ...
                    if status == "completed":
                        if output_file_id:
                            ...
                            # completed + 파일 있음 → 굳이 로그 안 찍어도 됨
                        else:
                            completed_without_output += 1
                            if not silent:
                                self.append_log(
                                    f"{bid} 상태: completed 이지만 output 파일이 아직 없습니다 "
                                    f"(output_file_id 없음)"
                                )
                    else:
                        not_completed += 1
                        if not silent:
                            self.append_log(
                                f"{bid} 상태: {status!r} (아직 완료되지 않아 결과 파일이 없습니다)"
                            )
            ...
            if not silent:
                self.append_log(
                    "갱신 완료 "
                    f"(완료+파일 {completed_with_output}건, "
                    f"완료/파일없음 {completed_without_output}건, "
                    f"진행중/기타 {not_completed}건)"
                )
    ...
    def _run_merge_multi(self, ids):
        ...
        for bid in ids:
            ...
            try:
                ...
                # ✅ [추가] 원격 상태 & output_file_id 체크
                remote_status = getattr(remote, "status", None)
                output_file_id = getattr(remote, "output_file_id", None)

                if remote_status != "completed":
                    self.append_log(
                        f"❌ {bid} 병합 불가: 원격 상태가 'completed'가 아니라 '{remote_status}' 입니다."
                    )
                    # DB에도 실제 상태 갱신
                    upsert_batch_job(bid, status=remote_status, output_file_id=output_file_id)
                    continue

                if not output_file_id:
                    self.append_log(
                        f"❌ {bid} 병합 불가: output_file_id 가 없습니다. "
                        f"(배치가 아직 완료되지 않았거나, 실패/취소/만료된 작업일 수 있습니다.)"
                    )
                    upsert_batch_job(bid, status=remote_status, output_file_id=None)
                    continue
                ...
                if local_job and local_job.get("src_excel"):
                    ...
                    # ✅ [수정] JSONL 출력은 원본 엑셀과 같은 폴더에 생성
                    src_dir = os.path.dirname(src_path)
                    base_name, _ = os.path.splitext(os.path.basename(src_path))
                    out_jsonl = os.path.join(src_dir, f"{base_name}_stage1_batch_output.jsonl")
                    
                    # [유지] 텍스트 파이프라인 T버전 증가 (_T1_I0.xlsx 형태)
                ...
                if src_path and os.path.exists(src_path):
                    ...
                    for cid, val in results_map.items():
                        try:
                            ...
                            # 여기서는 조용히 넘겨도 되고,
                            # 디버깅 원하면 로그 한줄 남겨도 됨 (너무 시끄러우면 pass 유지)
                            pass
                    ...
                    # ▼▼▼ [여기만 추가하세요] ▼▼▼
                    try:
                        # 1. 원본 파일명 기반으로 Root Name 추출
                        root_name = get_root_filename(src_path)
                        
                        # 2. 메인 런처 현황판 업데이트 (Text: Stage 1 완료)
