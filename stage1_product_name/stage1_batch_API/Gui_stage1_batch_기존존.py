...
Stage 1 Batch API 실행 스크립트 (GUI) - Final Fixed Version
- 기능: 엑셀 원본 -> Batch JSONL 생성(상품명 정제) -> 업로드 -> 실행 -> 병합 -> 정제 리포트
- 설정: GPT-5 계열 전용 (기본 gpt-5-mini / effort=low)
- 개선:
  1. 모듈 부재 시 명확한 에러 발생 (조용한 실패 방지)
  2. History 기록 시 비용 누적 문제 해결
  3. 수동 병합 시 상태 체크 안전장치 추가
"""
...
# [필수 의존성] batch_stage1_core / stage1_run_history
# 파일이 없으면 명확하게 에러를 발생시키거나 더미 처리
try:
    from batch_stage1_core import (
        safe_str,
        build_stage1_batch_payload, 
    )
except ImportError:
    # [Fix 1] 조용한 실패 방지: safe_str은 동작하되, build 함수는 에러 발생
    def safe_str(x):
        if x is None: return ""
        try:
    ...
    def build_stage1_batch_payload(*args, **kwargs):
        raise RuntimeError("필수 모듈 'batch_stage1_core.py'가 없습니다.\n같은 폴더에 해당 파일이 있는지 확인해주세요.")
...
class Stage1BatchGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stage 1: Batch API Manager (GPT-5 & Robust)")
    ...
    def _show_help_dialog(self):
        msg = (
            ...
            "1. [배치 생성 탭]: 원본 엑셀 파일을 선택하고 'Start Batch'를 클릭하세요.\n"
            "   - 원본상품명을 읽어 정제 요청(JSONL)을 생성합니다.\n"
            "   - 'gpt-5-mini (low)' 모델이 가성비가 뛰어납니다.\n\n"
            "2. [배치 관리 탭]: 'completed' 상태가 되면 [선택 일괄 병합]을 하세요.\n"
            "   - [자동 갱신]을 켜두면 주기적으로 상태를 확인합니다.\n"
            "   - [정제 리포트]를 통해 원본 vs 정제 결과를 비교할 수 있습니다.\n\n"
    ...
    def _init_tab_create(self):
        ...
        ToolTip(cb_model, "Stage 1은 gpt-5-mini를 추천합니다.")
        ...
        ToolTip(cb_effort, "단순 정제는 'low' 권장.")
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            # [Fix 2] 실행 파일 위치(BASE_DIR)에 저장
            jsonl_path = os.path.join(BASE_DIR, f"{base_name}_stage1_batch_input.jsonl")
    ...
    def _auto_refresh_loop(self):
        ...
        if self.auto_refresh_var.get() and not self.is_refreshing:
            # merged 포함된 완료 상태들은 제외 (불필요한 호출 방지)
            jobs = load_batch_jobs()
            active_ids = [
    ...
    def _run_merge_multi(self, ids):
            ...
            try:
                ...
                if src_path and os.path.exists(src_path):
                    ...
                    append_run_history(
                        ...
                        total_cost_usd=cost, # 여기는 해당 배치의 비용
        ...
        # 로그에는 누적 비용 표시
        self.append_log(f"=== 일괄 병합 끝 (성공: {success_cnt}, 총비용: ${total_cost:.4f}) ===")
        self._load_jobs_all()
    ...
    def _run_report_unified(self, ids):
        ...
        try:
            ...
            # [Fix] BASE_DIR 경로 사용
            report_path = os.path.join(BASE_DIR, f"Stage1_Clean_Report_{ts}.xlsx")
            pd.DataFrame(all_reps).to_excel(report_path, index=False)
            self.append_log(f"📊 리포트 완료: {os.path.basename(report_path)}")
            
            self.after(0, lambda: self._ask_open_file(report_path))
