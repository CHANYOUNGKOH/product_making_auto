"""
bg_Batch_prompt_gui_gemini.py

Stage 3-2: 전처리 -> AI배경생성 분석 (배치/대량) - Gemini Batch API 버전
...
# Core 모듈 import
try:
    from bg_prompt_core_gemini import (
...
BATCH_JOBS_FILE = os.path.join(BASE_DIR, "bg_prompt_batch_jobs_gemini.json")
...
# GUI Class
# ========================================================
class BGPromptGeminiBatchGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stage 3-2: 배경 프롬프트 생성 (Gemini Batch API - 50% 할인)")
        ...
        if not CORE_AVAILABLE:
            messagebox.showerror("오류", "bg_prompt_core_gemini.py 모듈을 찾을 수 없습니다.")
            self.destroy()
            return
    ...
    def _show_help_dialog(self):
        msg = (
            "[Stage 3-2 Gemini Batch API 사용 가이드]\n\n"
            "1. [배치 생성 탭]:\n"
            "   - T2 이상 _I3 버전 엑셀 파일을 선택하세요.\n"
    ...
    def _init_tab_create(self):
        ...
        # Step 1: 파일
        f_file = ttk.LabelFrame(container, text="1. 작업 대상 파일 선택 (T2 이상 _I3 버전만 가능)", padding=15)
        ...
        ToolTip(btn_file, "Stage 3-2를 수행할 I3 버전 엑셀 파일을 선택하세요.\n(ST2_JSON, view_point 컬럼 필수)")
        ...
        chk_skip = ttk.Checkbutton(fr2, text=" 이미 bg_positive_en이 있는 행은 건너뛰기", variable=self.skip_exist_var)
    ...
    def _select_src_file(self):
        ...
        if p:
            ...
            self.append_log(f"파일 선택됨: {base_name} (T{t_version}_I{i_version})")
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            jsonl_path = os.path.join(os.path.dirname(src), f"{base_name}_bg_prompt_gemini_batch_input.jsonl")

            self.append_log("JSONL 생성 중...")
            ...
            uploaded_file_name = upload_jsonl_file(
                client=client,
                jsonl_path=jsonl_path,
                display_name=f"{base_name}_bg_prompt_batch"
            )
            ...
            batch_info = create_batch_job(
                ...
                display_name=f"bg_prompt_{base_name}"
            )
            ...
            try:
                ...
                JobManager.update_status(root_name, img_s3_2_msg="I3-2 (배치 진행중)")
            except Exception:
                pass
    ...
    def _run_merge(self, names):
        ...
        try:
            ...
            for batch_name in names:
                ...
                output_jsonl = os.path.join(base_dir, f"{base_name}_bg_prompt_gemini_batch_output.jsonl")
                ...
                # 병합
                output_excel = get_i4_output_path(src_excel)
                ...
                try:
                    ...
                    JobManager.update_status(root_name, img_s3_2_msg="I3-2 (완료)")
                except Exception:
                    pass
