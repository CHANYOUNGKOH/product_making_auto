...
DEFAULT_SETTINGS_FILE = os.path.join(os.path.dirname(__file__), ".bg_prompt_batch_defaults.json")
...
def save_default_settings(model, effort):
    """기본 설정값 저장"""
    settings = {
        "model": model,
        "effort": effort
...
class BGPromptBatchGUI(tk.Tk):
    ...
    def _init_ui(self):
        ...
        ToolTip(btn_save_defaults, "현재 설정된 모델과 추론 강도를 기본값으로 저장합니다.\n다음 실행 시 자동으로 불러옵니다.")
    ...
    def _save_defaults(self):
        ...
        if save_default_settings(model, effort):
            messagebox.showinfo(
                "저장 완료", 
                f"기본값이 저장되었습니다:\n\n"
                f"• 모델: {model}\n"
