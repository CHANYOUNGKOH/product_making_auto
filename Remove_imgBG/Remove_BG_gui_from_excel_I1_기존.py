...
def get_root_filename(filename):
    """파일명에서 버전 정보(_T*_I*)를 제거하고 Root Key 추출"""
    name = os.path.basename(filename)
    base, ext = os.path.splitext(name)
    # _T숫자_I숫자 패턴 제거
    base = re.sub(r"_T\d+_I\d+$", "", base)
    # 구형 꼬리표 제거
    suffixes = ["_stage1_mapping", "_stage1_img_mapping", "_with_images", "_bg"]
    for s in suffixes:
        base = base.replace(s, "")
    return base + ext
...
class HybridBGApp:
    ...
    def start_processing(self):
        ...
        # [NEW] 다음 버전(_T*_I*) 파일명 계산
        self.target_excel_path = get_next_version_path(excel_path, task_type='image')
        
        # [NEW] 런처 현황판 업데이트 (시작 알림)
        try:
            root_name = get_root_filename(excel_path)
            # 현재 I버전+1 (예: I0 -> I1) 진행중 표시
            next_i = re.search(r"_I(\d+)", os.path.basename(self.target_excel_path))
            ver_str = f"I{next_i.group(1)}" if next_i else "Img"
            JobManager.update_status(root_name, img_msg=f"{ver_str} (진행중)")
        except: pass
    ...
    def worker(self, out_root, items, preset_name):
        ...
        # [MODIFIED] 엑셀 저장 로직 변경 (새 버전 파일로 저장)
        if self.df is not None:
            try:
                # 원본 덮어쓰기 대신 계산된 새 경로(T0_I1 등)에 저장
                self.df.to_excel(self.target_excel_path, index=False)
                self.ui_queue.put(("log", f"[저장] 새 버전 엑셀 생성 완료: {os.path.basename(self.target_excel_path)}"))
                
                # [NEW] 런처 현황판 업데이트 (완료 알림)
                if not stopped_by_user:
                    try:
                        root_name = get_root_filename(self.excel_path)
                        next_i = re.search(r"_I(\d+)", os.path.basename(self.target_excel_path))
                        ver_str = f"I{next_i.group(1)}" if next_i else "Img"
                        JobManager.update_status(root_name, img_msg=f"{ver_str} (완료)")
                    except: pass
