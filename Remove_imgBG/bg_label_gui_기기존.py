...
"""
bg_label_gui.py  (Excel DB + bg_mapping 호환)

배경제거 결과(bg_mapping.csv/json 또는 Excel DB)를 보면서
썸네일 품질 라벨(좋음/애매/불량)을 붙이는 전용 GUI.

지원 입력 형식:
  - CSV / JSON / JSONL (기존 bg_mapping 형식)
  - XLSX / XLS (Stage1/DB 엑셀)

엑셀에서 자주 쓰이는 컬럼명을 자동으로 매핑:
  - 카테고리      : [excel_카테고리명, 카테고리명, 카테고리]
  - ST1_상품명    : [excel_ST1_결과상품명, ST1_결과상품명, 결과상품명]
  - 원본상품명    : [excel_원본상품명, 원본상품명, 공급사상품명]
  - 마켓상품명    : [excel_마켓상품명, 마켓상품명, 노출상품명]
  - 상품코드      : [excel_상품코드, 상품코드, 판매자관리코드, 자사상품코드]
  - 상세설명(HTML): [excel_본문상세설명, 본문상세설명, 상세설명, 상세페이지]
  - 입력이미지    : [input_abs, 썸네일경로, 대표이미지, 대표이미지경로]
  - 출력이미지    : [output_abs, 배경제거경로, 썸네일_BG]

라벨 저장:
  - 파일이 CSV/JSON이면 → 기존처럼 그 파일에 human_label/human_notes 컬럼 저장
  - 파일이 XLSX/XLS이면 → 같은 엑셀 파일에 human_label/human_notes 컬럼 생성/갱신
"""
...
import pandas as pd  # ⬅ Excel 읽기/쓰기용 추가
...
# --------------------------------------------------------------------
CATEGORY_COL      = "카테고리명"
RESULT_NAME_COL   = "ST1_결과상품명"
ORIG_NAME_COL     = "원본상품명"
MARKET_NAME_COL   = "마켓상품명"
CODE_COL          = "상품코드"
DETAIL_HTML_COL   = "본문상세설명"
...
# Stage1 휴먼 라벨링 컬럼 (엑셀 컬럼명 기준)
LABEL_COL         = "IMG_S1_휴먼라벨"
NOTE_COL          = "IMG_S1_휴먼노트"
...
# --- 엑셀 헤더 후보들 ----------------------------------------------------
CATEGORY_CANDS = [CATEGORY_COL, "카테고리명", "카테고리"]
RESULT_CANDS   = [RESULT_NAME_COL, "ST1_결과상품명", "결과상품명", "ST1상품명"]
ORIG_CANDS     = [ORIG_NAME_COL, "원본상품명", "공급사상품명", "원상품명"]
MARKET_CANDS   = [MARKET_NAME_COL, "마켓상품명", "노출상품명", "스마트스토어상품명"]
CODE_CANDS     = [CODE_COL, "상품코드", "판매자관리코드1", "자사상품코드"]
DETAIL_CANDS   = [DETAIL_HTML_COL, "본문상세설명", "상세설명", "상세페이지", "상세"]

INPUT_PATH_CANDS  = [INPUT_PATH_COL, "썸네일경로", "대표이미지", "대표이미지경로"]
OUTPUT_PATH_CANDS = [OUTPUT_PATH_COL, "IMG_S1_누끼", "썸네일_BG", "썸네일누끼", "BG경로"]
...
def _build_rows_from_excel(path: str) -> List[Dict[str, Any]]:
    """
    Excel Stage1/DB 파일을 읽어서
    내부용 rows(list[dict]) 구조로 변환.
    """
    ...
    # 각 역할별 실제 컬럼명 탐색
    col_cat    = _find_first_col(df, CATEGORY_CANDS)
    col_res    = _find_first_col(df, RESULT_CANDS)
    col_orig   = _find_first_col(df, ORIG_CANDS)
    col_market = _find_first_col(df, MARKET_CANDS)
    col_code   = _find_first_col(df, CODE_CANDS)
    col_detail = _find_first_col(df, DETAIL_CANDS)

    col_in_path  = _find_first_col(df, INPUT_PATH_CANDS)
    col_out_path = _find_first_col(df, OUTPUT_PATH_CANDS)
    ...
    for i in range(len(df)):
        ...
        if col_cat:
            r[CATEGORY_COL] = _cell_to_str(df.at[i, col_cat])
        else:
            r[CATEGORY_COL] = ""

        if col_res:
            r[RESULT_NAME_COL] = _cell_to_str(df.at[i, col_res])
        else:
            r[RESULT_NAME_COL] = ""

        if col_orig:
            r[ORIG_NAME_COL] = _cell_to_str(df.at[i, col_orig])
        else:
            r[ORIG_NAME_COL] = ""

        if col_market:
            r[MARKET_NAME_COL] = _cell_to_str(df.at[i, col_market])
        else:
            r[MARKET_NAME_COL] = ""

        if col_code:
            r[CODE_COL] = _cell_to_str(df.at[i, col_code])
        else:
            r[CODE_COL] = ""

        if col_detail:
            r[DETAIL_HTML_COL] = _cell_to_str(df.at[i, col_detail])
        else:
            r[DETAIL_HTML_COL] = ""

        # 이미지 경로
        if col_in_path:
            r[INPUT_PATH_COL] = _cell_to_str(df.at[i, col_in_path])
        else:
            r[INPUT_PATH_COL] = ""

        if col_out_path:
            r[OUTPUT_PATH_COL] = _cell_to_str(df.at[i, col_out_path])
        else:
            # 배경제거 이미지가 아직 없다면 빈값
            r[OUTPUT_PATH_COL] = ""

        # rel 경로는 일단 비워두거나, 필요하면 여기서 매핑
        r[INPUT_REL_COL] = ""
        r[OUTPUT_REL_COL] = ""

        # 라벨 / 메모가 이미 엑셀에 있다면 가져옴
        r[LABEL_COL] = ""
        r[NOTE_COL] = ""

        if LABEL_COL in df.columns:
            r[LABEL_COL] = _cell_to_str(df.at[i, LABEL_COL])
        if NOTE_COL in df.columns:
            r[NOTE_COL] = _cell_to_str(df.at[i, NOTE_COL])
...
def load_mapping_file(path: str) -> List[Dict[str, Any]]:
    """
    Excel Stage1/DB 파일(xlsx)만 읽어서
    내부용 rows(list[dict]) 구조로 변환.
    """
    ext = os.path.splitext(path)[1].lower()

    if ext != ".xlsx":
        raise ValueError("이 도구는 .xlsx 파일만 지원합니다.")
...
def save_mapping_file(path: str, rows: List[Dict[str, Any]]):
    """
    엑셀(xlsx)의 IMG_S1_휴먼라벨 / IMG_S1_휴먼노트 컬럼만 업데이트.
    (행 인덱스를 기준으로 1:1 매핑)
    """
    ext = os.path.splitext(path)[1].lower()

    if ext != ".xlsx":
        raise ValueError("이 도구는 .xlsx 파일만 저장할 수 있습니다.")
...
class BgLabelApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("BG 라벨링 도구 (bg_mapping/Excel 기반)")
        # 전체화면 X, 고정 기본 크기
        ...
        self.path_var = tk.StringVar(value="bg_mapping/엑셀 파일을 열어주세요.")
        self.position_var = tk.StringVar(value="0 / 0")

        # 이미지 기준 폴더 (기본: 파일이 있는 폴더)
    ...
    def _build_ui(self):
        ...
        # 누끼제거 결과
        mid_frame = tk.LabelFrame(top_images_frame, text="누끼제거 결과", padx=4, pady=4)
        ...
        tk.Button(
            right_panel,
            text="저장 (엑셀 xlsx)  Ctrl+S",
            command=self.on_save,
            width=22,
        ).grid(row=1, column=0, sticky="e", padx=4, pady=(4, 4))
    ...
    def on_save(self):
        if not self.file_path or not self.rows:
            messagebox.showwarning("주의", "먼저 파일을 여세요.")
            return
        try:
