...
⚠️ 이 파일은 기존 버전입니다. 캐싱 최적화 버전은 stage2_batch_api_Cachever.py를 사용하세요.
"""
...
# stage2_batch_api_기존gpt.py는 이미 캐싱 최적화가 적용되어 있음 (CACHE_MODE 플래그로 제어)
try:
...
class Stage2BatchGUI(tk.Tk):
    def __init__(self):
        ...
        self.max_image_height_var = tk.IntVar(value=2000)  # 기본값: 최대 이미지 높이 2000px (토큰 절감)
        self.jpeg_quality_var = tk.IntVar(value=85)  # 기본값: JPEG 품질 85 (1-100)
    ...
    def _init_tab_create(self):
        ...
        ttk.Label(fr4, text="이미지 최대 높이:", width=18).pack(side='left')
        spin_max_height = ttk.Spinbox(fr4, from_=500, to=6000, textvariable=self.max_image_height_var, width=10, increment=100)
        spin_max_height.pack(side='left', padx=5)
        ToolTip(spin_max_height, "긴 이미지(4000-6000px)를 지정한 높이로 리사이징합니다.\n예: 2000px로 설정하면 세로가 긴 이미지를 2000px로 줄여서 토큰을 크게 절감합니다.\n가로는 비율을 유지합니다. 0 또는 None으로 설정하면 리사이징 안 함.")
        ttk.Label(fr4, text="px (기본값: 2000, 0=리사이징 안 함)", font=("맑은 고딕", 9)).pack(side='left', padx=5)
        
        ttk.Label(fr4, text="JPEG 품질:", width=12).pack(side='left', padx=(20, 5))
        spin_quality = ttk.Spinbox(fr4, from_=50, to=100, textvariable=self.jpeg_quality_var, width=10)
        spin_quality.pack(side='left', padx=5)
        ToolTip(spin_quality, "JPEG 압축 품질 (50-100). 낮을수록 파일 크기 작아져 토큰 절감.\n85 권장 (품질과 용량의 균형). 100은 무압축.")
        ttk.Label(fr4, text="(기본값: 85)", font=("맑은 고딕", 9)).pack(side='left', padx=5)
    ...
    def _run_create_batch(self):
        ...
        try:
            ...
            if CACHE_MODE_BATCH:
                self.append_log(f"[INFO] 🚀 프롬프트 캐싱 최적화 모드 활성화 (토큰 비용 최대 90% 절감 가능)")
            ...
            # max_detail_images, max_image_height, jpeg_quality 파라미터 전달
            call_kwargs = {
                "excel_path": src,
            ...
            # 이미지 리사이징 옵션 (캐싱 모드와 무관하게 사용 가능)
            if "max_image_height" in params:
                call_kwargs["max_image_height"] = self.max_image_height_var.get() if hasattr(self, 'max_image_height_var') else None
            if "jpeg_quality" in params:
                call_kwargs["jpeg_quality"] = self.jpeg_quality_var.get() if hasattr(self, 'jpeg_quality_var') else 85
