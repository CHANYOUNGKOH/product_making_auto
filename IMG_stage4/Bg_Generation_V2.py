...
class BGGenerationGUI(tk.Tk):
        ...
        try:
            ...
            for log_path, log_mtime in log_files:
                try:
                    ...
                    with open(log_path, "r", encoding="utf-8") as f:
                        for line in f:
                            # [배치] [N/M] ✅ 처리 완료: BG_row{N}_...png 패턴 찾기
                            match = re.search(r'\[배치\]\s*\[\d+/\d+\]\s*✅\s*처리\s*완료:\s*(BG_row(\d+)_[^\s]+\.png)', line)
                            if match:
                                filename = match.group(1)
                                row_num = int(match.group(2))  # BG_row{N}에서 N 추출
            ...
            for row_idx, info in completed_rows_final.items():
                ...
                if filename in output_files:
                    ...
                    # 정확한 매칭 실패 시 부분 매칭 시도 (BG_row{N}_로 시작하는 파일 찾기)
                    row_num = row_idx + 1
                    prefix = f"BG_row{row_num}_"
    ...
    def _recover_bg_from_output_directory(self, df, comfyui_output_dir, excel_path):
        ...
        try:
            ...
            # BG_row{N}_ 패턴으로 시작하는 파일 찾기
            bg_files = {}
            for filename in output_files:
                if filename.endswith('.png') and filename.startswith('BG_row'):
                    # BG_row{N}_ 패턴 추출
                    match = re.match(r'BG_row(\d+)_', filename)
                    if match:
