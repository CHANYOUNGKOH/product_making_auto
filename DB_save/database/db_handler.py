...
# 시즌 설정 파일 캐싱 (성능 최적화)
_season_config_cache = None
_season_config_cache_path = None
...
class DBHandler:
    ...
    def get_products_for_upload(self, category: str, sheet_name: str, business_number: str, status: str = 'ACTIVE', exclude_assigned: bool = True, season_filter_enabled: bool = True, sheet_used_combinations: Dict[str, set] = None, store_used_product_codes: set = None, product_code_filter_mode: str = "none", product_code_filter_codes: set = None) -> List[Dict]:
        """
        ...
        - 상품코드 필터링 적용 (선택적, 성능 최적화를 위해 시즌 필터링 전에 적용)
        - 시즌 필터링 적용 (선택적)
        ...
        Args:
            ...
            sheet_used_combinations: 시트별 사용된 조합 (캐시된 데이터, None이면 새로 조회)
            store_used_product_codes: 스토어별 사용된 상품코드 (캐시된 데이터, None이면 새로 조회)
            product_code_filter_mode: 상품코드 필터 모드 ("none", "exclude", "include")
            product_code_filter_codes: 필터링할 상품코드 set (None이면 필터링 안 함)
        ...
        # 2. 이미 할당된 조합 인덱스 조회 (시트 전체) - 캐싱된 데이터가 없으면 조회
        if sheet_used_combinations is None:
            cursor.execute("""
        ...
        # 3. 스토어별로 이미 사용한 상품코드 확인 - 캐싱된 데이터가 없으면 조회
        if store_used_product_codes is None:
        ...
        # 4.5. 상품코드 필터링 적용 (시즌 필터링 전에 적용하여 성능 최적화)
        # 필터링 순서: 카테고리 조회 → 상품코드 필터링 → 시즌 필터링 → 조합 조회
        if product_code_filter_mode != "none" and product_code_filter_codes:
            original_product_count = len(products_with_info)
            original_product_codes_count = len(set(p.get("상품코드", "") for p in products_with_info))
            
            if product_code_filter_mode == "exclude":
                # 제외 모드: 입력한 상품코드 제외
                products_with_info = [p for p in products_with_info if p.get("상품코드", "") not in product_code_filter_codes]
                excluded_codes_count = original_product_codes_count - len(set(p.get("상품코드", "") for p in products_with_info))
            elif product_code_filter_mode == "include":
                # 포함 모드: 입력한 상품코드만 포함
                products_with_info = [p for p in products_with_info if p.get("상품코드", "") in product_code_filter_codes]
                excluded_codes_count = original_product_codes_count - len(set(p.get("상품코드", "") for p in products_with_info))
            else:
                excluded_codes_count = 0
            
            # 필터링 결과 저장 (로그 출력용 - 나중에 메인 윈도우에서 사용)
            if not hasattr(self, '_last_product_code_filter_info'):
                self._last_product_code_filter_info = {}
            self._last_product_code_filter_info[category] = {
                'mode': product_code_filter_mode,
                'original_count': original_product_count,
                'original_product_codes_count': original_product_codes_count,
                'filtered_count': len(products_with_info),
                'filtered_product_codes_count': len(set(p.get("상품코드", "") for p in products_with_info)),
