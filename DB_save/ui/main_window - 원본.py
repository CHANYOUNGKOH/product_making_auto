...
class MainWindow(tk.Tk):
    ...
    def _init_ui(self):
        ...
        # 2. DB 파일 선택 (컴팩트)
        frame_db = ttk.LabelFrame(file_frame, text="💾 데이터베이스 파일", padding=10)
        ...
        selection_frame.pack(fill='both', expand=True, pady=(0, 8))  # expand=True로 높이 확보
        
        # 3-1. 카테고리 선택 (왼쪽)
        self.frame_category = ttk.LabelFrame(selection_frame, text="📂 카테고리 선택 (대>중)", padding=10)
        ...
        self.category_info_label.pack(anchor='w', pady=(0, 5))
        
        # 카테고리 트리뷰와 스크롤바
        cat_tree_frame = ttk.Frame(self.frame_category)
        cat_tree_frame.pack(fill='both', expand=True)  # expand=True로 높이 확보
        
        self.category_tree = ttk.Treeview(cat_tree_frame, show='tree', selectmode='none', height=12)  # 높이 증가
        ...
        cat_btn_frame.pack(fill='x', pady=(8, 0))
        ...
        quantity_info_frame.pack(fill='x', pady=(0, 5))
        ...
        self.frame_markets = ttk.LabelFrame(selection_frame, text="🏪 마켓/스토어 선택", padding=10)
        # 초기 모드가 "upload"이므로 표시
        ...
        self.market_tree = ttk.Treeview(tree_frame, show='tree headings', selectmode='none', height=12, columns=("memo", "product_count", "registered_count"))
        ...
        season_export_frame.pack(fill='x', pady=(5, 0))
        ...
        self.btn_start.pack(fill='x', ipady=8, pady=(5, 0))
        ...
        self.log_frame = ttk.LabelFrame(main_frame, text="📝 진행 로그", padding=10)
        self.log_frame.pack(fill='both', expand=True, pady=(0, 0))  # expand=True로 남은 공간 모두 사용
        
        self.log_widget = ScrolledText(self.log_frame, height=25, state='disabled',  # 높이 증가
...
"""
    ...
    def _update_store_display(self, store_id: str):
        ...
        if memo_categories:
            ...
            product_count_str = f"{total_count}개"
    ...
    def _update_tree_checkboxes(self):
        ...
        for item_id, var in self.tree_checkboxes.items():
            ...
            # 스토어 노드는 메모 정보 포함하여 표시
            if item_data.get("type") == "store":
                self._update_store_display(item_id)
            else:
                # 일반 노드는 기존 방식대로
    ...
    def _export_for_upload(self, selected_categories: List[str], selected_markets: List[Dict[str, str]]):
        ...
        threading.Thread(target=self._run_export_for_upload, args=(selected_categories, selected_markets, save_dir, exclude_assigned, export_mode), daemon=True).start()
    ...
    def _re_export_from_history(self, selected_categories: List[str], selected_markets: List[Dict[str, str]], save_dir: str, exclude_assigned: bool = True, export_mode: str = "upload", custom_filename: str = None, skip_logging: bool = False):
        ...
        try:
            self._run_export_for_upload(selected_categories, selected_markets, save_dir, exclude_assigned, export_mode, custom_filename, skip_logging)
        finally:
            # 정리
    ...
    def _run_export_for_upload(self, selected_categories: List[str], selected_markets: List[Dict[str, str]], save_dir: str, exclude_assigned: bool = True, export_mode: str = "upload", custom_filename: str = None, skip_logging: bool = False):
            ...
            for sheet_name, sheet_markets in markets_by_sheet.items():
                ...
                for market_info in sheet_markets:
                        ...
                        if season_filter_enabled:
                            if hasattr(db_handler, '_last_season_filter_info') and db_handler._last_season_filter_info:
                                ...
                                if 'error' in season_info:
                                    ...
                                    self._log(f"      - 최종 사용 가능: {total_after}개 (실제 반환: {actual_returned}개)")
                    ...
                    for product_code in product_codes_list:
                        ...
                        product_combinations = all_products_by_code[product_code]
                        
                        # 사용 가능한 조합 찾기 (전체 시트에서 사용되지 않은 조합 중에서)
                        assigned = False
                        found_product = None
                        found_combination_key = None
                        
                        for candidate_product in product_combinations:
                            mix_url = candidate_product.get("믹스url", "") or ""
                            nukki_url = candidate_product.get("누끼url", "") or ""
                            final_name = candidate_product.get("ST4_최종결과", "") or ""
                            url_type = candidate_product.get("url_type", "mix")
                            line_index = candidate_product.get("line_index", 0)
                            ...
                            # 사용 가능한 조합 발견
                            assigned = True
                            found_product = candidate_product
                            found_combination_key = combination_key
                            break
                        
                        # 사용 가능한 조합이 없으면 스킵 (이 상품코드의 모든 조합이 이미 사용됨)
                        if not assigned:
                            continue
                        ...
                        if combination_index is not None:
                            try:
                                ...
                                if cursor.rowcount == 0:
                                    # 이미 할당된 조합이면 다른 조합 찾기
                                    self._log(f"    ⚠️ 조합 {combination_index}이 이미 할당됨, 다른 조합 찾는 중...")
                                    # 다음 조합으로 재시도는 복잡하므로 일단 스킵
                                    continue
                                
                                db_handler.conn.commit()
                            except Exception as e:
                                db_handler.conn.rollback()
                                self._log(f"    ⚠️ 조합 할당 기록 실패: {product_code} - {e}")
                                continue  # 할당 실패 시 해당 상품 스킵
                        ...
                        try:
                            ...
                            if exclude_assigned and not skip_logging:
                                db_handler.log_upload(
                                    business_number=business_number,
                                    market_id=market_id,
                                    market_name=sheet_name,  # 시트명으로 기록
                                    product_id=product_id,
                                    product_code=product_code,
                                    used_product_name=final_name,
                                    used_nukki_url=used_nukki_url,
                                    used_mix_url=used_mix_url,
                                    product_name_index=product_name_index,
                                    image_nukki_index=image_nukki_index,
                                    image_mix_index=image_mix_index,
                                    upload_strategy=json.dumps(strategy, ensure_ascii=False),
                                    upload_status="SUCCESS",
                                    notes=f"카테고리: {', '.join(store_categories) if store_categories else 'N/A'}, 마켓: {market_name}, 스토어별칭: {alias}, 줄번호: {line_index}"
                                )
                                market_logged_count += 1
                            # exclude_assigned가 False이거나 skip_logging이 True이면 기록하지 않음
                            ...
                            self._log(f"    ⚠️ DB 기록 실패: {product_code} - {e}")
                            import traceback
                            self._log(traceback.format_exc())
