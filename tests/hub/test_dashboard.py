...
def test_dashboard_returns_stats(client):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["total_all"] == 4        # 전체 4개
    assert data["total_active"] == 3     # ACTIVE 3개
    assert data["text_done"] == 2        # W001, W002 (ST4_마켓상품명 있음)
    assert data["image_done"] == 1       # W001 (누끼+연출 모두)
    assert data["image_partial"] == 1    # W002 (누끼만)
    assert data["shippable"] == 2        # W001, W002 (텍스트+누끼+가격 모두)
    assert "last_sync_at" in data


def test_dashboard_stat_values_are_ints(client):
    data = client.get("/api/dashboard").json()
    for key in ("total_active", "text_done", "image_done", "image_partial", "shippable"):
...
def test_dashboard_has_processing_stats(client):
    ...
    data = r.json()
    for key in ["text_done", "image_done", "image_partial", "shippable",
